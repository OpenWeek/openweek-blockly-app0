...
def turnLeft():
    global PLAYER_ORIENTATION, TURNS
...
try:
    for i in range(len(data["map"]["layout"])):
        ...
        if isOnTarget() and notDone():
            #print("Vous y êtes presque ! Votre presonnage atteint le but mais il manque une action.", end='', flush=True)
